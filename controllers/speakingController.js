...
const getSavedSpeakingResults = asyncHandler(async (req, res, next) => {
  try {
    ...
    // }
    console.log("Speaking Results: ", speakingResults);
...
const saveResults = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (user.role === "student") {
      const testCount = await Speaking.countDocuments({
        user_id: req.user.uid,
      });
      if (testCount >= 2) {
        return res.status(403).json({
          message: "Free test limit exceeded. Please upgrade to premium.",
        });
      }
    }
